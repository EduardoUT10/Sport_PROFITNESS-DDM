...
namespace Sport.ViewModels
{
    ...
    public class EditExerciseViewModel : BaseViewModel
    {
        ...
        private string textDifficulty = "Fácil";
        ...
        public async void LoadItemId(int exerciseId)
        {
            try
            {
                ...
                Image = GetExercise.Source.Replace("Archivo: ", "");
                Description = GetExercise.Description;
                TextDifficulty = GetExercise.Difficulty;
            }
            catch (Exception)
            {
                Debug.WriteLine("Error al cargar la imagen");
            }
        }
        ...
        {
            var result = await App.Current.MainPage.DisplayActionSheet("Dificultad del Ejercicio", "Cancelar", null, "Facil", "Media", "Fuerte");
